...
def findPath(deposits, capacity):
    ...
    history1 = []
    solutions2, labels, clusterMatrix = generateMultipleSolutions(deposits, capacity, NR_SOLUTIONS)

    for i in range(0, ITERATIONS):
        crossOver(solutions2, clusterMatrix)
        if i%100 == 0  :
            regenerate(solutions2, deposits, capacity, labels, clusterMatrix)
        if i%10 == 0 :
            printBestSolution(solutions2, history1)
    # plt.plot(range(len(history)), history)
    # plt.show()
    mergeSort(solutions)
    mergeSort(solutions2)
    newSolutions = solutions[0:len(solutions)/2]
    newSolutions.extend(solutions2[0:len(solutions)/2])

    history = []

    for i in range(0, ITERATIONS):
        crossOver(newSolutions, clusterMatrix)
        if i%100 == 0  :
            regenerate(newSolutions, deposits, capacity, labels, clusterMatrix)
        if i%10 == 0 :
            printBestSolution(newSolutions, history)
